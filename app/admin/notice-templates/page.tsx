  ...
  return (
    <div className="max-w-[1280px] mx-auto space-y-lg">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-h1 font-semibold text-on-surface">사내 게시물 템플릿</h1>
          <p className="text-body-base text-on-surface-variant mt-xs">
            공지·캠페인용 배경 이미지 템플릿을 등록·관리합니다. 사용자는 텍스트만 수정해서 사용합니다.
          </p>
        </div>
        <Link
          href="/admin/notice-templates/new"
      ...
      {!mounted ? (
        ...
        <div className="bg-white p-xl rounded-xl card-shadow border border-outline-variant/30 text-center">
          ...
          <h3 className="text-h3 font-semibold mb-xs">등록된 사내 게시물 템플릿이 없습니다</h3>
          <p className="text-body-base text-on-surface-variant mb-lg">
            첫 템플릿을 등록하면 사용자가 "디자인 제작 → 사내 게시물 제작" 에서 선택할 수 있습니다.
          </p>
          <Link
            href="/admin/notice-templates/new"
        ...
        <div className="grid grid-cols-3 gap-lg">
          {items.map((t) => {
            const isUser = isUserNoticeTemplate(t);
                ...
                <div className="p-md">
                  ...
                  <div className="flex items-center gap-xs pt-md border-t border-outline-variant">
                    {isUser ? (
                      <>
                        <Link
                          href={`/admin/notice-templates/${t.id}`}
