...
import {
  ...
  uploadNoticeTemplateImage,
  type NoticeTemplateInput,
} from "@/lib/store/notice-templates";

const MAX_IMAGE_MB = 8;

type Preset = { label: string; width: number; height: number };
const PRESETS: Preset[] = [
  { label: "A4 세로 (827×1170)", width: 827, height: 1170 },
  { label: "A4 가로 (1170×827)", width: 1170, height: 827 },
  { label: "정사각 (1080×1080)", width: 1080, height: 1080 },
  { label: "인스타 스토리 (1080×1920)", width: 1080, height: 1920 },
  { label: "가로 공지 (1600×900)", width: 1600, height: 900 },
];

type Props = {
  mode: "create" | "edit";
  initial?: NoticeTemplate;
  createdBy?: string;
};

export function NoticeTemplateForm({ mode, initial, createdBy }: Props) {
  ...
  const fallbackSlots = computeDefaultSlots(initial?.width ?? 1080, initial?.height ?? 1080);
  ...
  function applyFile(f: File) {
    setError(null);
    if (!f.type.startsWith("image/")) return setError("이미지 파일만 가능합니다.");
    if (f.size > MAX_IMAGE_MB * 1024 * 1024)
      return setError(`파일이 ${MAX_IMAGE_MB}MB를 초과합니다.`);
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (file) {
        const uploaded = await uploadNoticeTemplateImage(file);
        if (!uploaded) {
      ...
      const payload: NoticeTemplateInput = {
        name: name.trim(),
      ...
      let result;
      if (mode === "create") {
        result = await createNoticeTemplate(payload);
      } else if (initial) {
        result = await updateNoticeTemplate(initial.id, payload);
      }
      if (!result) {
        setError("저장에 실패했습니다.");
        setSubmitting(false);
      ...
      router.push("/admin/notice-templates");
      router.refresh();
    } catch (err) {
      console.error(err);
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-[900px] mx-auto space-y-lg">
      <div className="flex items-center gap-xs text-secondary text-label-sm">
        <Link href="/admin/notice-templates" className="hover:text-primary transition-colors">
          사내 게시물 템플릿
        </Link>
      ...
      <h1 className="text-h1 font-semibold text-on-surface">
        {mode === "create" ? "새 사내 게시물 템플릿" : "템플릿 수정"}
      </h1>
